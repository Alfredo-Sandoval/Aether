(() => {
  const AetherContentSurfaceTools = Object.freeze({
    createSurfaceTools(deps = {}) {
      ...
      const isSettingsSurfaceDescriptor = requireDependency("isSettingsSurfaceDescriptor");
      const isProjectSurfaceDescriptor = requireDependency("isProjectSurfaceDescriptor");
      ...
      const isSettingsShellVisible = () => {
        ...
        return panels.some((panel) => isSettingsSurfaceDescriptor(panel));
      };

      const isProjectShellPath = () => {
        return PROJECT_SHELL_PATH_PATTERN.test(location.pathname.toLowerCase());
      };

      const isProjectShellVisible = () => {
        if (isProjectShellPath()) return true;
        const panels = collectVisibleSnapshotNodes("main, section, article, div[role='dialog']", 6, isLargeShellNode);
        return panels.some((panel) => isProjectSurfaceDescriptor(panel));
      };

      const detectChatShellVariant = () => {
        if (document.querySelector('a[aria-current="page"][href*="/gg/"]')) return "group-chat";
        const pathname = location.pathname.toLowerCase();
        if (pathname.includes("/gg/")) return "group-chat";
        if (pathname.includes("/c/")) return "standard-chat";
        return "generic-chat";
      };
      ...
      const detectShellKind = () => {
        ...
        if (isProjectShellPath() || isProjectShellVisible()) return "project";
      ...
      const buildShellSnapshot = () => {
        return {
          ...
          variant: detectChatShellVariant(),
      ...
      const captureDomSurfaceSnapshot = () => {
        ...
        return {
          ...
          summary: {
            ...
            dialogCount: countVisibleNodes(`.popover[role="dialog"], div[role="dialog"], ${ACTIVITY_FLYOUT_SELECTOR}`),
      ...
      const getSnapshotSignature = (snapshot) => {
        return JSON.stringify({
          ...
          shell: {
            ...
            variant: snapshot.shell.variant,
      ...
      const hasShellSnapshotChanged = (beforeSnapshot, afterSnapshot) => {
        return (
          ...
          beforeSnapshot.shell.variant !== afterSnapshot.shell.variant ||
