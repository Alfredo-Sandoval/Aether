...
const collectRepoFiles = (repoRoot) => {
  const collected = [];
  walkFiles(repoRoot, repoRoot, collected);
  return collected;
};

const addPathIfPresent = (repoRoot, entry, collected) => {
...
const addPatternMatches = (repoRoot, pattern, repoFiles, collected) => {
  if (!pattern || typeof pattern !== "string") return;
  if (!pattern.includes("*")) {
    addPathIfPresent(repoRoot, pattern, collected);
    return;
  }

  const matcher = new RegExp(`^${pattern.replace(/[.+?^${}()|[\]\\]/g, "\\$&").replace(/\*/g, "[^/]+")}$`);
  const matches = repoFiles.filter((entry) => matcher.test(entry));
  if (matches.length === 0) {
...
const collectPopupReferencedEntries = (repoRoot, collected) => {
  const popupPath = path.join(repoRoot, "popup.html");
  if (!fs.existsSync(popupPath)) {
    throw new Error("Missing popup.html referenced by manifest action popup");
  }
  ...
  for (const match of popupHtml.matchAll(assetPattern)) {
    ...
    addPathIfPresent(repoRoot, target, collected);
  }
};
...
const collectExpectedEntries = (repoRoot) => {
  ...
  const expected = [];
  addPathIfPresent(repoRoot, "manifest.json", expected);
  addPathIfPresent(repoRoot, "LICENSE", expected);

  addPathIfPresent(repoRoot, manifest.background?.service_worker, expected);
  addPathIfPresent(repoRoot, manifest.action?.default_popup, expected);

  Object.values(manifest.icons || {}).forEach((entry) => addPathIfPresent(repoRoot, entry, expected));
  Object.values(manifest.action?.default_icon || {}).forEach((entry) => addPathIfPresent(repoRoot, entry, expected));

  (manifest.content_scripts || []).forEach((contentScript) => {
    (contentScript.js || []).forEach((entry) => addPathIfPresent(repoRoot, entry, expected));
    (contentScript.css || []).forEach((entry) => addPathIfPresent(repoRoot, entry, expected));
  });

  (manifest.web_accessible_resources || []).forEach((resourceBlock) => {
    (resourceBlock.resources || []).forEach((entry) => addPatternMatches(repoRoot, entry, repoFiles, expected));
  });

  if (manifest.default_locale) {
    addPatternMatches(repoRoot, "_locales/*/messages.json", repoFiles, expected);
  }

  collectPopupReferencedEntries(repoRoot, expected);
